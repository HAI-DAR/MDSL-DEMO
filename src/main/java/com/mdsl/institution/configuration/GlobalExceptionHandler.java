...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler{
    ...
    public ResponseEntity<ApiErrorResponse> handleBusinessException(BusinessException ex, HttpServletRequest request) throws BaseException {
	log.error(ex.getMessage(), ex);
        return constructTranslatedErrorResponse(CustomHttpStatus.BUSINESS_ERROR.value(),ex.getMsgCode(), ex.getMessage(), request.getRequestURI() );
    }
    ...
    private ResponseEntity<ApiErrorResponse> constructGenericErrorResponse(
	    String errorMessage,
	    String path
	    )
    {
    ...
    private ResponseEntity<ApiErrorResponse> constructTranslatedErrorResponse(
	    Integer statusCode,
	    Integer msgCode,
	    String msgDesc,
	    String path
    ...
    private ApiErrorResponse appendTranslatedMessageData(ApiErrorResponse response, Integer msgCode, String msgDesc) throws BaseException
    {
	...
	switch (lang) {
        	...
        	response.setErrorDesc(StringUtil.nullEmptyToValue(message.getMsgDescriptionAr(), message.getMsgDescriptionEn()) );
        	}
            case FRENCH -> {
        	response.setErrorTitle(StringUtil.nullEmptyToValue(message.getMsgTitleFr(), message.getMsgTitleEn() ) );
        	response.setErrorDesc(StringUtil.nullEmptyToValue(message.getMsgDescriptionFr(), message.getMsgDescriptionEn()) );
        	}
            default -> {
        	response.setErrorTitle(message.getMsgTitleEn());
        	response.setErrorDesc(message.getMsgDescriptionEn());
